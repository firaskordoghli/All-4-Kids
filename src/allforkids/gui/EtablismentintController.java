/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package allforkids.gui;

import com.jfoenix.controls.JFXButton;
import java.io.IOException;
import java.net.URL;
import java.util.ResourceBundle;
import javafx.event.ActionEvent;
import javafx.fxml.FXML;
import javafx.fxml.FXMLLoader;
import javafx.fxml.Initializable;
import javafx.scene.Parent;
import javafx.scene.Scene;
import javafx.stage.Stage;

/**
 * FXML Controller class
 *
 * @author FATNASSI
 */
public class EtablismentintController implements Initializable {

    @FXML
    private JFXButton responsable;
    @FXML
    private JFXButton eleve;
    @FXML
    private JFXButton parent;
    @FXML
    private JFXButton admin;

    /**
     * Initializes the controller class.
     */
    @Override
    public void initialize(URL url, ResourceBundle rb) {
        // TODO
    }    

    @FXML
    private void responsable(ActionEvent event) throws IOException {
          Stage stage = new Stage();
        stage.setTitle("Espace responsable");
        Parent root = FXMLLoader.load(getClass().getResource("Etablisment.fxml"));
        Scene scene = new Scene(root);
        stage.setScene(scene);
        stage.show();
    }

    @FXML
    private void eleve(ActionEvent event) throws IOException {
        Stage stage = new Stage();
        stage.setTitle("Espace éleve");
        Parent root = FXMLLoader.load(getClass().getResource("Eleves.fxml"));
        Scene scene = new Scene(root);
        stage.setScene(scene);
        stage.show();
    }

    @FXML
    private void parent(ActionEvent event) throws IOException {
        Stage stage = new Stage();
        stage.setTitle("Espace parent");
        Parent root = FXMLLoader.load(getClass().getResource("Parent.fxml"));
        Scene scene = new Scene(root);
        stage.setScene(scene);
        stage.show();
    }

    @FXML
    private void admin(ActionEvent event) throws IOException {
        Stage stage = new Stage();
        stage.setTitle("Espace admin");
        Parent root = FXMLLoader.load(getClass().getResource("Admin.fxml"));
        Scene scene = new Scene(root);
        stage.setScene(scene);
        stage.show();
    }
    
}
